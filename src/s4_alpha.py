...
Components:
  ...
  event risk   — deterministic from calendar.days_to_earnings; missing = UNKNOWN,
                 never silently LOW.
  combine      — reads S3's predict.dir_1d per ticker and gates it: no
...
def run_alpha(universe: list[str], event_date: str,
    ...
    with Trigger("alpha", stage="S4") as trig:
        ...
        for t in universe:
            ...
            pred = store.read_asof("predict.dir_1d", t, event_date, as_known_at)
        ...
        trig.add_metrics(
            ...
            has_predictions=bool(universe and store.read_asof(
                "predict.dir_1d", universe[0], event_date)),
